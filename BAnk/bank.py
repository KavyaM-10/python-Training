...
def delete_account():
    ...
    id=input("Enter the id to delete account:").strip()
    if not id.isdigit():
        print("Please,Enter valid input id")
        return
    
    cursor=conn.cursor()
    cursor.execute("DELETE FROM bank_info WHERE id=%s",(id,))
...
def main():
    ...
    while True:
        print("1.Create Account,2.View Account,3.Update Account,4.delete,5.Exit")
